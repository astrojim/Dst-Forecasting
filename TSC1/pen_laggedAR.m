function penout = pen_laggedAR(x,y,xtol,ytol,lag,lagAR)
...
for iterYcompare = (1+lag):1:length(y),
    for iterYcheck = (1+lag):1:length(y),
        if( tolcheck(y(iterYcheck),y(iterYcompare),ytol) && tolcheck(x(iterYcheck-lag),x(iterYcompare-lag),xtol) && tolcheck(y(iterYcheck-lagAR),y(iterYcompare-lagAR),ytol)),
            pEyesC_count = pEyesC_count + 1;
        end;
        if( tolcheck(x(iterYcheck-lag),x(iterYcompare-lag),xtol) && tolcheck(y(iterYcheck-lagAR),y(iterYcompare-lagAR),ytol) ),
